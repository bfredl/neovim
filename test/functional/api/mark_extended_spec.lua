...
describe('Extmarks buffer api', function()
  ...
  it('substitions with newline in match and sub, delta is 0 #extmarks2', function()
    feed('A<cr>67890<cr>xx<esc>')
    buffer('set_mark', buf, ns, marks[1], 1, 4)
    buffer('set_mark', buf, ns, marks[2], 1, 5)
    buffer('set_mark', buf, ns, marks[3], 1, 6)
    buffer('set_mark', buf, ns, marks[4], 2, 1)
    buffer('set_mark', buf, ns, marks[5], 2, 6)
    buffer('set_mark', buf, ns, marks[6], 3, 1)
    feed([[:1,1s:5\n:\r<cr>]])
    check_undo_redo(buf, ns, marks[1], 1, 4, 1, 4)
    check_undo_redo(buf, ns, marks[2], 1, 5, 2, 1)
    check_undo_redo(buf, ns, marks[3], 1, 6, 2, 1)
    check_undo_redo(buf, ns, marks[4], 2, 1, 2, 1)
    check_undo_redo(buf, ns, marks[5], 2, 6, 2, 6)
    check_undo_redo(buf, ns, marks[6], 3, 1, 3, 1)
  end)

  it('substitions with newline in match and sub, delta > 0 #fail', function()
    feed('A<cr>67890<cr>xx<esc>')
    buffer('set_mark', buf, ns, marks[1], 1, 4)
    buffer('set_mark', buf, ns, marks[2], 1, 5)
    buffer('set_mark', buf, ns, marks[3], 1, 6)
    ...
    buffer('set_mark', buf, ns, marks[6], 3, 1)
    feed([[:1,1s:5\n:\r\r<cr>]])
    -- after_newline_in_sub needs to be calculated correctly
    -- check_undo_redo(buf, ns, marks[1], 1, 4, 1, 4)
    -- check_undo_redo(buf, ns, marks[2], 1, 5, 3, 1)
    -- check_undo_redo(buf, ns, marks[3], 1, 6, 3, 1)
    -- check_undo_redo(buf, ns, marks[4], 2, 1, 3, 1)
    -- check_undo_redo(buf, ns, marks[5], 2, 6, 3, 6)
    -- check_undo_redo(buf, ns, marks[6], 3, 1, 4, 1)
  end)

  it('substitions with newline in match and sub, delta < 0 #extmarks2', function()
    ...
    buffer('set_mark', buf, ns, marks[4], 2, 1)
    buffer('set_mark', buf, ns, marks[5], 2, 6)
    buffer('set_mark', buf, ns, marks[6], 3, 2)
    buffer('set_mark', buf, ns, marks[7], 4, 1)
    feed([[:1,2s:5\n.*\n:\r<cr>]])
    check_undo_redo(buf, ns, marks[1], 1, 4, 1, 4)
    check_undo_redo(buf, ns, marks[2], 1, 5, 2, 1)
    check_undo_redo(buf, ns, marks[3], 1, 6, 2, 1)
    check_undo_redo(buf, ns, marks[4], 2, 1, 2, 1)
    check_undo_redo(buf, ns, marks[5], 2, 6, 2, 1)
    check_undo_redo(buf, ns, marks[6], 3, 2, 2, 2)
    check_undo_redo(buf, ns, marks[7], 4, 1, 3, 1)
  end)
