...
describe('Extmarks buffer api', function()
  ...
  it('querying for information and ranges #extmarks', function()
    ...
    eq({}, curbufmeths.get_extmarks(ns, {0, 0}, {0, 0}, ALL))
  ...
  it('get_marks works when mark col > upper col #extmarks', function()
    ...
    eq({{marks[1], 0, 4},
        {marks[2], 1, 1},
        {marks[3], 2, 0}},
       curbufmeths.get_extmarks(ns, {0, 3}, {2, 1}, -1))
  ...
  it('get_marks works in reverse when mark col < lower col #extmarks', function()
    ...
    rv = curbufmeths.get_extmarks(ns, {0, 2}, {2, 3}, -1, 1)
    eq({{marks[1], 2, 1},
        {marks[2], 1, 4},
        {marks[3], 0, 2}},
       rv)
  ...
  it('using <c-a> when increase in order of magnitude #extmarks', function()
    ...
    feed('<c-a>')
    check_undo_redo(ns, marks[1], 0, 0, 0, 0)
    check_undo_redo(ns, marks[2], 0, 2, 0, 3)
  end)
  ...
  it('using <c-x> when decrease in order of magnitude #extmarks', function()
    ...
    check_undo_redo(ns, marks[1], 0, 4, 0, 3)
    -- TODO do we need to test marks[1] ???
    -- check_undo_redo(ns, marks[2], 0, 2, 0, 2)
  end)
...
describe('Extmarks buffer api', function()
  ...
  it("can get marks", function()
    local marks1 = curbufmeths.get_extmarks(ns1, {0,0}, {-1, -1}, -1)
    eq(233, #marks1)
    local marks2 = curbufmeths.get_extmarks(ns2, {0,0}, {-1, -1}, -1)
    eq(232, #marks2)
  end)
