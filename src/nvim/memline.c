...
enum {
  ...
  // always stored in the byte order 'b' '0', so file can be identified
...
bool swapfile_fname_differs(char *swap_fname, char *buf_ffname)
{
  int fd;
  ZeroBlock b0;
  int differ = false;
  ...
  fd = os_open(swap_fname, O_RDONLY, 0);
  if (fd >= 0) {
    if (read_eintr(fd, &b0, sizeof(b0)) == sizeof(b0)) {
      ...
      if (b0.b0_flags & B0_SAME_DIR) {
        if (path_fnamecmp(path_tail(buf_ffname),
                          path_tail(b0.b0_fname)) != 0
            || !same_directory(swap_fname, buf_ffname)) {
          // Symlinks may point to the same file even
          // when the name differs, need to check the
          // inode too.
          expand_env(b0.b0_fname, NameBuff, MAXPATHL);
          if (fnamecmp_ino(buf_ffname, NameBuff,
...
time_t swapfile_info(char *fname)
{
  assert(fname != NULL);
  int fd;
  ZeroBlock b0;
...
bool swapfile_unchanged(char *fname)
{
  ZeroBlock b0;
