...
{
  assert(!fp->wr);
  size_t in_buffer = (size_t)(fp->write_pos - fp->read_pos);

  size_t from_buffer = MIN(in_buffer, size);
  ...
  if (fp->eof) {
    ...
    if (r_ret < 0) {
      ...
      size_t to_copy = MIN(r_ret, read_remaining);
      memcpy(ret_buf, fp->read_pos, to_copy);
      ret_buf += to_copy;
      read_remaining = size - from_buffer;
    }
  }
#endif
...
ptrdiff_t file_skip(FileDescriptor *const fp, const size_t size)
  FUNC_ATTR_NONNULL_ALL
{
  // TODO: rewrite to be good and not bad and no longer need skipbuf
  assert(!fp->wr);
  size_t read_bytes = 0;
  do {
