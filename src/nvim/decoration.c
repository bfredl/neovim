...
// TODO(bfredl): These should really be per-buffer, so that all resources
                             ...
                             colnr_T offset)
{
  ...
  for (linenr_T lnum = pos_start.lnum; lnum <= pos_end.lnum; lnum++) {
    ...
    extmark_set(buf, (uint32_t)src_id, NULL,
                ...
                decor, kDecorLevelVisible, true, false, true, false, NULL);
  }
}
...
bool decor_redraw_start(win_T *wp, int top_row, DecorState *state)
{
  ...
  while (marktree_itr_step_overlap(buf->b_marktree, state->itr, &pair)) {
    MTKey m = pair.start;
    if (mt_invalid(m) || marktree_decor_level(m) < kDecorLevelVisible) {
      continue;
    }
                        ...
                        int *cul_id, int *num_id)
{
  ...
  while (marktree_itr_step_overlap(buf->b_marktree, itr, &pair)) {
    if (!mt_invalid(pair.start) && true) {  // TODO: quick skipover
  ...
  while (itr->x) {
    ...
    if (!mt_end(mark) && !mt_invalid(mark) && true) { // TODO: sqkipover
...
int decor_signcols(buf_T *buf, int row, int end_row, int max)
{
  ...
  for (int currow = row; currow <= end_row; currow++) {
    ...
    MTPair pair;
    while (marktree_itr_step_overlap(buf->b_marktree, itr, &pair)) {
      // TODO: replace "decor levels" with bitflags, one of which is gonna be "sign with text"
