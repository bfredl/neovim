...
{
  DLOGN("RPC %s %" PRIu64 ": %s id=%u: %s", dir, channel_id, REQ, req_id, name);
}

static void log_response(char *dir, uint64_t channel_id, char *kind, uint32_t req_id)
{
  DLOGN("RPC %s %" PRIu64 ": %s id=%u", dir, channel_id, kind, req_id);
}

static void log_notify(char *dir, uint64_t channel_id, const char *name)
{
  DLOGN("RPC %s %" PRIu64 ": %s %s", dir, channel_id, NOT, name);
}
...
bool rpc_send_event(uint64_t id, const char *name, Array args)
{
  ...
  log_notify(SEND, channel->id, name);
  if (channel) {
...
static void serialize_request(Channel** chans, size_t nchans, uint32_t request_id, const char *method,
                                  Array args)
{
  PackerBuffer packer;
...
void serialize_response(Channel *channel, MsgpackRpcRequestHandler handler,
...
static void packer_buffer_init_channels(Channel **chans, size_t nchans, PackerBuffer *packer) {
  ...
  packer->flush = channel_flush_callback;
...
static void channel_flush_callback(PackerBuffer *packer, size_t size_hint) {
  packer_buffer_finish_channels(packer);
