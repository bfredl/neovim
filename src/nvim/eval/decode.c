...
static void negative_integer_to_special_typval(typval_T *rettv, int64_t val) {
  assert(val < 0);
  if (val >= VARNUMBER_MIN) {
    *rettv = (typval_T) {
      .v_type = VAR_NUMBER,
      .v_lock = VAR_UNLOCKED,
      .vval = { .v_number = (varnumber_T)val },
    ...
    // TODO(bfredl): this code will be deaded if varnumber_T is
    // guaranteed to be at least int64_t forever
    list_T *const list = tv_list_alloc(4);
    tv_list_ref(list);
    create_special_dict(rettv, kMPInteger, ((typval_T) {
      .v_type = VAR_LIST,
  ...
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  case MSGPACK_OBJECT_POSITIVE_INTEGER:
    positive_integer_to_special_typval(rettv, mobj.via.u64);
    break;
  case MSGPACK_OBJECT_NEGATIVE_INTEGER:
    negative_integer_to_special_typval(rettv, mobj.via.i64);
    break;
...
msgpack_to_vim_generic_map: {}
  ...
  case MSGPACK_OBJECT_EXT: {
    ...
    create_special_dict(rettv, kMPExt, ((typval_T) { .v_type = VAR_LIST,
                                                     .v_lock = VAR_UNLOCKED,
                                                     .vval = { .v_list = list } }));
    encode_list_write((void *)ext_val_list, mobj.via.ext.ptr, mobj.via.ext.size);
    break;
  }
  }
  return OK;
...
static void typval_parse_enter(mpack_parser_t *parser, mpack_node_t *node)
{
  ...
  case MPACK_TOKEN_SINT: {
    // TODO(bfredl): all of MPACK_TOKEN_SINT will just be a single assignment
    // if varnumber_T is at least int64_t
    int64_t value = mpack_unpack_sint(node->tok);
    if (value < 0) {
      negative_integer_to_special_typval(result, value);
    } else {
      positive_integer_to_special_typval(result, (uint64_t)value);
    }
    break;
                         }
  ...
  case MPACK_TOKEN_EXT:
    ...
    // fprintf(stderr, "ENTER: %p %p with len %ul\n", (void *)node, node->data[1].p, node->tok.length);
    break;
...
static void typval_parse_exit(mpack_parser_t *parser, mpack_node_t *node)
{
  ...
  switch (node->tok.type) {
    case MPACK_TOKEN_BIN:
    case MPACK_TOKEN_STR:
    // fprintf(stderr, "EXITa: %p %p with len %ul\n", (void *)node, node->data[1].p, node->tok.length);
...
// does not support stream unpacking (restart after EOF), as we don't need to
int typval_from_msgpack(const char** data, size_t *size, typval_T *ret)
{
  ...
  if (status != MPACK_OK) {
    // TODO: how do we not leak memory of typval_parse_exit temporaries on error exit :p
    tv_clear(ret);
  }
  return status;
}
