...
void ins_char_bytes(char *buf, size_t charlen)
{
  ...
  if (State & REPLACE_FLAG) {
    ...
    replace_push_str(oldp + col, oldlen);
  }
...
bool open_line(int dir, int flags, int second_line_indent, bool *did_do_comment)
{
  ...
  if (State & VREPLACE_FLAG) {
    ...
    replace_push_char(NUL);      // Call twice because BS over NL expects it
    replace_push_char(NUL);
    p = saved_line + curwin->w_cursor.col;
    replace_push_str(p, strlen(p));
  ...
  if (p_extra != NULL) {
    *p_extra = saved_char;              // restore char that NUL replaced
    ...
    if (REPLACE_NORMAL(State)) {
      replace_push_char(NUL);            // end of extra blanks
    }
    if (curbuf->b_p_ai || (flags & OPENLINE_DELSPACES)) {
      while ((*p_extra == ' ' || *p_extra == '\t')
             && !utf_iscomposing_first(utf_ptr2char(p_extra + 1))) {
        if (REPLACE_NORMAL(State)) {
          replace_push_char((uint8_t)*p_extra);
        }
        p_extra++;
        less_cols_off++;
      }
