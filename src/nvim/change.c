...
static void changed_common(buf_T *buf, linenr_T lnum, colnr_T col, linenr_T lnume, linenr_T xtra)
{
  ...
  // TODO: should be (made) redundant!
  if (curwin->w_buffer == buf && curwin->w_p_diff && diff_internal()) {
...
{
  buf_changed_lines(curbuf, lnum, col, lnume, xtra, do_buf_event);
}
...
void buf_changed_lines(buf_T *buf, linenr_T lnum, colnr_T col, linenr_T lnume, linenr_T xtra, bool do_buf_event)
{
  buf_redraw_changed_lines_later(buf, lnum, lnume, xtra);

  // TODO: super crufty! what if buffer was edited in real-curwin
  // and the same buffer was part of a diff split in two other windows??
  // how did that even work today (redundant) or did it not
