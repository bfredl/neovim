...
static void buf_set_sign(buf_T *buf, uint32_t *id, char *group, int prio, linenr_T lnum, sign_T *sp)
{
  ...
  extmark_set(buf, ns, id, lnum - 1, 0, -1, -1, decor, kDecorLevelVisible, true,
...
static int buf_delete_signs(buf_T *buf, char *group, int id, linenr_T atlnum)
{
  ...
  if (atlnum > 0) {
    ...
    while (marktree_itr_step_overlap(buf->b_marktree, itr, &pair)) {
      if ((ns == UINT32_MAX || ns == pair.start.ns) && decor_find_sign(mt_decor(pair.start))) {
  ...
  while (itr->x) {
    ...
    if (!mt_end(mark) && decor_find_sign(mt_decor(mark))
        && (id == 0 || (int)mark.id == id)
...
static void sign_list_append_info(sign_T *sp, list_T *retlist)
{
...
/// Append dictionary of information for a defined sign "sp", or placed
/// sign "mark" to "retlist". Either "sp", or "mark" is NULL.
static void sign_list_append_info_mark(MTKey mark, list_T *retlist)
{
  dict_T *d = tv_dict_alloc();
  tv_list_append_dict(retlist, d);
    ...
    FUNC_ATTR_NONNULL_RET FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  while (itr->x) {
    MTKey mark = marktree_itr_current(itr);
    if (!mt_end(mark) && decor_find_sign(mt_decor(mark))) {
      sign_list_append_info_mark(mark, l);
    }
    marktree_itr_next(buf->b_marktree, itr);
  }
                                   ...
                                   list_T *retlist)
{
  ...
  while (itr->x) {
    ...
    if (!mt_end(mark)
      ...
      if (decor_find_sign(mt_decor(mark))) {
        kv_push(signs, mark);
      }
    }
    marktree_itr_next(buf->b_marktree, itr);
  ...
  if (kv_size(signs)) {
    qsort((void *)&kv_A(signs, 0), kv_size(signs), sizeof(MTKey), sign_cmp);
    for (size_t i = 0; i < kv_size(signs); i++) {
      sign_list_append_info_mark(kv_A(signs, i), l);
    }
    kv_destroy(signs);
  }
}
...
void f_sign_getdefined(typval_T *argvars, typval_T *rettv, EvalFuncData fptr)
{
  ...
  if (argvars[0].v_type == VAR_UNKNOWN) {
    map_foreach_value(&sign_map, sp, {
      sign_list_append_info(sp, rettv->vval.v_list);
    });
  } else {
