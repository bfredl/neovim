...
static void draw_col_ascii(winlinevars_T *wlv, const char *text, int width, int attr) {
  for (int i = 0; i < width; i++) {
    linebuf_char[wlv->off] = schar_from_ascii(text[i]);
...
#define FIX_FOR_BOGUSCOLS \
  ...
  while (true) {
    ...
    if (((dollar_vcol >= 0
         ...
         || (number_only)  // TODO: stop earlier && wlv.draw_state > WL_STC)
         )
        && wlv.filler_todo <= 0) {
    ...
    const bool draw_folded = has_fold && wlv.row == startrow + wlv.filler_lines;  // TODO: light check 
    ...
    // options are set
    vcol_save_attr = -1;
    if (true // TODO: WL_BRI and WL_SBR
        && !lnum_in_visual_area
        && search_attr == 0
        && area_attr == 0
        && wlv.filler_todo <= 0) {
    ...
    if (wlv.skip_cells <= 0) { // TODO: likely BRI/SBR mess
      // Store the character.
    ...
    if (wlv.col >= grid->cols && (!has_fold || virt_line_offset >= 0)
      ...
      // TODO: not here!
      if (draw_cols && wlv.filler_todo <= 0) {
