...
void pum_redraw(void)
{
  ...
  // TODO: this changes often, allow the logical size to be smaller than
  // the allocated size!
  if (!pum_grid.chars
      || pum_grid.Rows != pum_height || pum_grid.Columns != grid_width) {
    grid_alloc(&pum_grid, pum_height, grid_width, false);
    ui_call_grid_resize(pum_grid.handle, pum_grid.Columns, pum_grid.Rows);
  }

  compositor_put_grid(&pum_grid, pum_row, pum_col-col_off, false);
