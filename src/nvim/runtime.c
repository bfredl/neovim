...
ArrayOf(String) runtime_get_named(bool lua, Array pat, bool all)
{
  ...
  ArrayOf(String) rv = runtime_get_named_common(false, lua, pat, all, path, buf, sizeof buf);
...
ArrayOf(String) runtime_get_named_thread(bool lua, Array pat, bool all)
{
  ...
  static char buf[MAXPATHL];
  ArrayOf(String) rv = runtime_get_named_common(true, lua, pat, all, runtime_search_path_thread,
                                                buf, sizeof buf);
  uv_mutex_unlock(&runtime_search_path_mutex);
  return rv;
}

ArrayOf(String) runtime_get_named_common(bool thread, bool lua, Array pat, bool all,
  ...
  for (size_t i = 0; i < kv_size(path); i++) {
    ...
    if (lua) {
      if (item->has_lua == kNone) {
        ...
        item->has_lua = (size < buf_len && os_isdir_thread(thread, buf));
      }
    ...
    for (size_t j = 0; j < pat.size; j++) {
      ...
      if (pat_item.type == kObjectTypeString) {
        ...
        if (size < buf_len) {
          if (os_file_is_readable(thread, buf)) {
