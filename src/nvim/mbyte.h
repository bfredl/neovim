  ...
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_ALWAYS_INLINE FUNC_ATTR_PURE
{
  ...
  if (EXPECT(*next < 0x80U, true)) {
    int cells = cur.chr.value < 0x80 ? ascii2cells(cur.chr.value) : utf_char2cells(cur.chr.value);
    return (ClusterInfo) {
      .next = (StrCharInfo){
        .ptr = (char *)next,
...
static inline ClusterInfo utf_ClusterInfo_or_NUL(StrCharInfo cur)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_ALWAYS_INLINE FUNC_ATTR_PURE
{
