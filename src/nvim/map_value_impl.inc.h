...
// Don't error yet. it is nice to type-check the file in isolation, in clangd or otherwise
// but make sure to error on the last line
#define KEY_NAME(x) x##int
#define VAL_NAME(x) quasiquote(x, ptr_t)
const ptr_t value_init_ptr_t = NULL;
#define JUST_TESTING
#endif
...
VALUE_TYPE MAP_NAME(map_get_)(MAP_TYPE *map, KEY_TYPE key)
{
...
#ifdef JUST_TESTING
  STATIC_ASSERT_STATEMENT(false, "map_impl.inc.h included without definitions");
#endif
}
