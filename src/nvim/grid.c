...
// cached glyphs are just appended to a string with NUL separators
// thus we can look them up by memory offset
...
// buf need not be NUL terminated, but `buf` may not contain embedded NULS.
...
/// check if cache is full, and if full, clear it.
///
...
/// only for testing. The condition in schar_cache_clear_force is hard to
/// reach, so we force the clear in a test to make sure it works.
