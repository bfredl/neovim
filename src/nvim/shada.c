...
#define SEARCH_PAT_VAL(name, loc) \
  ...
  case kSDItemRegister: {
    ...
    if (!unpack_keydict(&it, DictHash(shada_register), &ad, &read_ptr, &read_size, &error_alloc)) {
...
#define REGISTER_VAL(name, loc, type) \
  ...
  case kSDItemBufferList: {
    ...
    for (size_t i = 0; i < (size_t)len; i++) {
      ...
      if (!unpack_keydict(&it, DictHash(shada_buflist_item), &it_ad, &read_ptr, &read_size,
                          &error_alloc)) {
