...
// Specialized version of Set() where interned strings is stored in a compact, NUL-separated
// char array. Used to cache glyphs which doesn't fit an a sizeof(schar_T) length UTF-8 string
...
uint32_t mh_put_glyph(Set(glyph) *set, String key, MHPutStatus *new)
{
  ...
  if (mh_is_either(h, idx)) {
    ...
    uint32_t size = (uint32_t)key.size + 1;  // NUL take space
    ...
    *new = kMHExisting;
    ...
    if (!equal_String(cstr_as_string(&set->keys[pos]), key)) {
      abort();
    }
    return pos;
  }
