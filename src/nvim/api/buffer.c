...
#define OPTION_TO_BOOL(target, name) \
    target = api_object_to_bool(opts. name, #name, false, err); \
    if (ERROR_SET(err)) { \
      goto error; \
    }

  OPTION_TO_BOOL(decor.virt_text_hide, virt_text_hide);
  OPTION_TO_BOOL(decor.hl_eol, hl_eol);
  ...
  OPTION_TO_BOOL(right_gravity, right_gravity);
  ...
  OPTION_TO_BOOL(end_right_gravity, end_right_gravity);

  size_t len = 0;

  bool ephemeral = false;
  OPTION_TO_BOOL(ephemeral, ephemeral);
