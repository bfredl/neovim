  ...
  FUNC_API_SINCE(9)
{
  ...
  Array patterns = get_patterns_from_pattern_or_buf(opts->pattern, has_buffer, opts->buffer, "*", arena, err);
  if (ERROR_SET(err)) {
    goto cleanup;
  }
...
static Array get_patterns_from_pattern_or_buf(Object pattern, bool has_buffer,
                                             Buffer buffer, char *fallback, Arena *arena, Error *err)
{
