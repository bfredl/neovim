  ...
  FUNC_API_NOEXPORT
{
  ...
  if (!channel) {
    // this function should only be called in --embed-ui mode, stdio channel
    // can be assumed.
    abort();
  }

  LOOP_PROCESS_EVENTS_UNTIL(&main_loop, channel->events, -1, pmap_has(uint64_t)(connected_uis, CHAN_STDIO));
}
