...
static Array extmark_to_array(MTPair extmark, bool id, bool add_dict, bool hl_name)
{
  ...
  if (add_dict) {
    ...
    put_decor_to_dict_legacy(&dict, mt_decor(start), hl_name);
  ...
  FUNC_API_SINCE(7)
{
  ...
  if (opts->ephemeral && decor_state.win && decor_state.win->w_buffer == buf) {
    ...
    } else if (decor.ext || has_hl) {
      decor_level = kDecorLevelVisible;
    }

    extmark_set(buf, (uint32_t)ns_id, &id, (int)line, (colnr_T)col, line2, col2,
                decor, decor_level, right_gravity, opts->end_right_gravity,
  ...
  FUNC_API_SINCE(1)
{
  ...
  extmark_set(buf, ns, NULL, (int)line, (colnr_T)col_start, end_line, (colnr_T)col_end,
              decor, kDecorLevelVisible, true, false, false, false, NULL);
  return ns_id;
}
