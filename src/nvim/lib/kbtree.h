...
#define __KB_ITR(name, key_t, kbnode_t) \
	...
	static int kb_getp_itr_##name(kbtree_##name##_t *b, const key_t * __restrict k, kbitr_##name##_t *itr) \
	{																	\
		int i, r = 0;													\
		itr->p = 0; \
	...
	static inline int kb_get_itr_##name(kbtree_##name##_t *b, const key_t k, kbitr_##name##_t *itr) \
	{																	\
