...
pub fn build(b: *std.Build) !void {
    ...
    const sysconfig_step = b.addConfigHeader(.{ .style = .{ .cmake = b.path("cmake.config/config.h.in") } }, .{
        ...
        .HAVE_FORKPTY = modernUnix and !tag.isDarwin(), // TODO: also on darwin but we lack the headers :(
        ...
        .VTERM_TEST_FILE = "test/vterm_test_output", // TODO: icky! this should be set at runtime..
    });
    ...
    // TODO: b.getInstallStep() is borked. rebuilds too much.
    ...
    const test_prerequisites = b.step("test_deps", "test prerequisites");
    test_prerequisites.dependOn(&nvim_exe_install.step);
    test_prerequisites.dependOn(&runtime_install.step);

    test_prerequisites.dependOn(test_fixture(b, "shell-test", null, target, optimize));
    test_prerequisites.dependOn(test_fixture(b, "tty-test", libuv, target, optimize));
    test_prerequisites.dependOn(test_fixture(b, "pwsh-test", null, target, optimize));
    test_prerequisites.dependOn(test_fixture(b, "printargs-test", null, target, optimize));
    test_prerequisites.dependOn(test_fixture(b, "printenv-test", null, target, optimize));
    test_prerequisites.dependOn(test_fixture(b, "streams-test", libuv, target, optimize));

    const parser_c = b.dependency("treesitter_c", .{ .target = target, .optimize = optimize });
    test_prerequisites.dependOn(add_ts_parser(b, "c", parser_c.path("."), false, target, optimize));
    const parser_markdown = b.dependency("treesitter_markdown", .{ .target = target, .optimize = optimize });
    test_prerequisites.dependOn(add_ts_parser(b, "markdown", parser_markdown.path("tree-sitter-markdown/"), true, target, optimize));
    test_prerequisites.dependOn(add_ts_parser(b, "markdown_inline", parser_markdown.path("tree-sitter-markdown-inline/"), true, target, optimize));
    const parser_vim = b.dependency("treesitter_vim", .{ .target = target, .optimize = optimize });
    test_prerequisites.dependOn(add_ts_parser(b, "vim", parser_vim.path("."), true, target, optimize));
    const parser_vimdoc = b.dependency("treesitter_vimdoc", .{ .target = target, .optimize = optimize });
    test_prerequisites.dependOn(add_ts_parser(b, "vimdoc", parser_vimdoc.path("."), false, target, optimize));
    const parser_lua = b.dependency("treesitter_lua", .{ .target = target, .optimize = optimize });
    test_prerequisites.dependOn(add_ts_parser(b, "lua", parser_lua.path("."), true, target, optimize));
    const parser_query = b.dependency("treesitter_query", .{ .target = target, .optimize = optimize });
    test_prerequisites.dependOn(add_ts_parser(b, "query", parser_query.path("."), false, target, optimize));

    try tests.test_steps(b, nvim_exe, test_prerequisites, lua_dev_deps.path("."), test_config_step.getDirectory());
}
