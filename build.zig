...
pub fn build(b: *std.Build) !void {
    ...
    const ziglua_host = if (cross_compiling) b.dependency("ziglua", .{
        .target = target_host,
        .optimize = optimize_lua,
        .lang = if (host_use_luajit) E.luajit else E.lua51, // TODO: bullll
        .shared = false,
    }) else ziglua;
    ...
    const nlua0 = build_nlua0(b, target_host, optimize_host, host_use_luajit, ziglua_host, lpeg);
    ...
    if (cross_compiling) {
        config_str = b.fmt("{s} -Dtarget={s} (host: {s})", .{ config_str, try t.linuxTriple(b.allocator), try b.graph.host.result.linuxTriple(b.allocator) });
    }
...
pub fn generate_header_for(
    ...
    run_step.addFileArg(b.path("src/nvim/generators/c_grammar.lua"));
    return run_step;
}
